...
**Goal:** 在 GitHub `origin/main` 的 SiteMark v1.0.8（`abc0164`）功能基线上，**从零**用原生 ArkTS 按 2026-08 最新 HarmonyOS 开发规范开发独立鸿蒙应用，交付可在 HarmonyOS NEXT 真机运行的签名 HAP，产品语义与 Android v1.0.8 **全量对等**；产品代码全部落在从 `abc0164` 拉出的长期分支 `ohos-native`。
...
**与既有 `ohos` 分支（社区 Flutter 适配路线）的关系：** 本计划**取代**该路线。`ohos` 分支截至 `821f3d4` 的实测结论是关键输入：① 空壳 Flutter HAP 仅在 OpenHarmony 6.0.2.130（API 22）**模拟器**冷启动通过，**无 NEXT 真机**验证；② Rust `ohos-arm64` 可交叉编译但引擎在设备侧初始化失败，长期处于 `degraded` 降级水印；③ 相册 ACL、相机、定位契约全部未在真机闭环。`ohos` 分支冻结归档仅作参考（发布日记按 `captureId`、条件清日记、串行队列、隐私弹窗文案等**设计结论直接继承**），不再推进；`ohos-native` 为唯一活跃鸿蒙线。
...
- 全量对等只能在以下条件**同时**成立时对外宣称：NEXT 真机手工回归总表全绿 + AGC 批准相册 ACL + Rust 水印版式与 Android v1.0.8 真机对照通过。任何降级（托底相册、降级水印、通知缺失）必须写入 `ohos-native/docs/deltas.md` 差异表并在发布说明标注，不得 silently 假装对等。
- 不在 UI、记录卡片、SnackBar/Toast、诊断包对外文案里暴露原始异常或平台字符串。
- 所有行为变更先写失败测试（hypium 先红），再写最小实现验证转绿；纯逻辑必须落在 `commons/*` HAR 以便单测。Task 0 工具链用真机冷启动作为验收，不编造单测替代表。
- 真机是硬闸：模拟器（含 DevEco NEXT 模拟器）只能用于日常开发，Task 0 与最终验收必须至少一台 HarmonyOS NEXT 真机（建议 5.x 与 6.x 各一台）。
...
| 16 | Hero 转场 / 玻璃材质 | ArkUI `sharedTransition` + 模糊/透明度材质容器 | 12 | 视觉近似即可，列入真机走查项 |
...
| 19 | 通知/内存压力清理 imageCache | PixelMap 列表缩略缓存上限（≈40 张/32MB 等价）+ `onMemoryLevel` 清理 | 12 | 真机走查滚动流畅性 |
...
| `ohos-native/entry/libs/arm64-v8a/libsitemark_core.so` | `rust/` 交叉编译产物（构建脚本拷贝，不入 LFS） |
| `rust/src/ffi/` | 新增 `extern "C"` JSON FFI 层（只增不改，FRB 导出不动） |
| `tool/ohos-native/` | `probe.md`（工具链/真机实测）、`build-rust.ps1`、`build-hap.ps1`、`deltas.md`（差异表） |
...
### Task 0: 规范基线、真机硬闸与 `ohos-native` 分支

**Files:**
- Create branch: `ohos-native` from `origin/main` @ `abc0164`（含本计划与配套规格的合并结果）
- Create: `tool/ohos-native/probe.md`（实测记录：DevEco 版本、SDK、真机型号与 API 版本、相机/相册/设置页/分享探测结论）
- Do not modify: `main` 的任何文件

**Interfaces:**
- Consumes: GitHub `origin/main` `abc0164`；本计划（经 PR 合入 `main` 后拉分支，或先 cherry-pick 本提交到 `ohos-native`）
- Produces: 可在 NEXT 真机冷启动的签名空壳 HAP；五项技术探测结论（相机 picker、相册直写/删除授权流、应用详情页跳转、文件保存托底、openLink）

- [ ] **Step 1: 安装并锁定工具链**

安装 DevEco Studio 6.1.1 Release（6.1.1.300）+ 内置 SDK（API 24）。`hvigorw -v`、`ohpm -v`、`hdc version` 输出写入 `probe.md`。开启 hvigor 守护进程与并行编译加速。

- [ ] **Step 2: 生成 Stage 模型空壳并跑通冷启动**

DevEco 新建 Empty Ability 工程（Stage 模型，ArkTS，phone），`targetSdkVersion 6.1.1(24)`、`compatibleSdkVersion 5.0.5(17)`；自动签名后真机安装冷启动。模拟器仅作日常开发用。

- [ ] **Step 3: 五项技术探测（结论写入 probe.md，后续任务按结论走）**

1. `cameraPicker.pick()` 拍照返回 URI 行为与权限要求（预期无需 CAMERA 权限）；
...
过关条件：真机冷启动成功 + 五项探测有明确结论。不过：停在 Task 0，把失败写入 `probe.md`，向用户汇报后调整（不得带着未探测项进入后续任务）。
...
- [ ] **Step 4: 真机验证**：首启弹隐私门 → 同意 → 进入三分支骨架；杀进程重启不再弹；语言切换即时生效。
...
- [ ] **Step 3: 验证**：hypium 全绿；真机确认建库、插入、分页翻页稳定（相同拍摄时间也能稳定翻页）。
...
- Create: `tool/ohos-native/build-rust.ps1`（`cargo build --release --target aarch64-unknown-linux-ohos` → 拷贝 .so 到 `entry/libs/arm64-v8a/`）
...
- [ ] **Step 3: 真机版式对照**：同字段同参数拍一张，与 Android v1.0.8 成片逐块对照（项目名/字段/时间/位置/透明度/字号/强调色/EXIF 方向）。结论写 `tool/ohos-native/engine_status.md`（`ok` / `degraded`）。
...
- [ ] **Step 2: 实现桥接与队列**，转绿。
- [ ] **Step 3: 真机手工**：建项目→填表→系统相机→确认入库；取消拍照；拍后立即杀进程再进（半截恢复）；拒定位仍出片；连拍编号递增。
...
- [ ] **Step 3: 真机手工**：ACL 授权路径直写相册；拒绝授权 → SaveButton 托底 + 「未进入系统相册」文案；删除整条记录时相册照片按授权语义处理（差异表登记）；再保存仅对 `ready` 且成片在；清理原图保留成片与记录；删除项目不删相册照片。
...
- [ ] **Step 2: 真机走查**：三分支状态保活（返回不闪现）、Dock 交互、Hero 转场、批量操作全链路、深浅色与强调色切换、中英切换。
...
- Create: `ohos-native/docs/deltas.md` 终版（后台处理/动态取色/相册删除授权/分享通道等全部差异）
...
- [ ] **Step 2: release 构建**：签名 HAP 产出，真机覆盖安装升级保留数据。
- [ ] **Step 3: 手工回归总表全绿**（见下）后，AGC 材料齐备提交审核。
- [ ] **Step 4: Commit** — `feat(ohos-native): release build, ci and appgallery materials`

---

## 手工回归总表（Task 8 收口时一次跑完，真机）
...
| 里程碑 | 内容 | 出口条件 |
| --- | --- | --- |
| M0 | Task 0 | 真机冷启动 + 五项探测落档 |
| M1 | Task 1–2 | 骨架 + 数据层黄金向量全绿 |
| M2 | Task 3–4 | 拍摄链路真机闭环 + 引擎 `ok/degraded` 定级 |
| M3 | Task 5 | 相册/媒体语义真机闭环（ACL 或已声明托底） |
| M4 | Task 6 | UI 逐屏对等走查通过 |
| M5 | Task 7 | 备份跨端互恢复通过 |
| M6 | Task 8 | 回归总表全绿 + AGC 材料齐 |

**全量对等宣称** = 真机回归总表全绿 ∧ ACL 获批且直写路径生效 ∧ 引擎 `ok` ∧ `deltas.md` 无未声明差异。任一不满足 → 发布说明必须按差异表降级表述。
...
**路线决策依据：** `ohos` 分支三项实测（模拟器-only、引擎 degraded、ACL 未闭环）证明社区 Flutter 路线无法在近期达到真机全量对等；原生 ArkTS + Rust 同源核心是对等性最强路径。`ohos` 分支的设计资产（日记/会话/队列/隐私文案/AGC 清单）已全部吸收。

**占位符扫描：** 相机 picker 行为、NAPI 绑定方式、应用详情页 Want、分享通道、compatibleSdkVersion 下限五处标注“以 Task 0/3 实测为准”并规定落档 `probe.md`，未留双实现。
