...
- [ ] 写失败测试：重复拖动水印滑块只调用一次项目读取。
- [ ] 写失败测试：纯函数正确处理项目、年、月、日半开区间筛选，且不修改输入列表。
- [ ] 写失败测试：全部记录和项目详情的筛选结果仍与现有行为一致。
- [ ] 缓存项目 Future 和页面 Stream；删除嵌套的第二/第三个照片 StreamBuilder。
- [ ] 运行上述测试并提交。
...
- [ ] 写失败测试：父组件重建不会重复读取同一照片的原图状态和输出路径。
- [ ] 写失败测试：文件存在性可异步返回，加载时显示稳定占位。
- [ ] 写失败测试：多个照片路径请求只解析、创建一次目录。
- [ ] 将卡片和预览改为有状态缓存；仅在相关照片字段变化时失效。
- [ ] 运行上述测试并提交。
...
- [ ] 写失败测试：普通详情预览具有受控 `cacheWidth`，列表缩略图仍为 192，全屏图片不限制。
- [ ] 根据布局宽度和 DPR 计算并限制解码宽度。
- [ ] 运行测试并提交。
...
- [ ] 写失败测试：离开并返回设置页不会自动重新统计，手动刷新仍会重新统计。
- [ ] 写失败测试：多文件统计结果正确且最大并发数不超过设计上限。
- [ ] 实现 Provider 缓存和有界并发读取。
- [ ] 运行测试并提交。
...
- [ ] 写失败迁移测试：v4 升级后数据保留且三个索引存在。
- [ ] 写失败新库测试：新建 v5 数据库立即具有三个索引。
- [ ] 增加 v5 迁移和幂等建索引逻辑，运行 Drift 代码生成。
- [ ] 增加 1000 条记录筛选测试并验证结果。
- [ ] 运行数据库测试并提交。
...
- [ ] 写失败测试：拍照工作流依次报告目标文件、待拍记录和系统相机启动三个阶段。
- [ ] 实现零网络、默认关闭的本地诊断计时接口。
- [ ] 增加 1000 条记录滚动、筛选、全选和详情打开的 Profile 集成场景。
- [ ] 运行工作流和集成测试并提交。
