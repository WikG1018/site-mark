...
**Source:** `docs/superpowers/plans/2026-07-25-settings-secondary-menu.md` (Task 5)

## Goal
Migrate the new-project watermark defaults (position + opacity + font scale + accent color) into its own sub-page. Uses `ConsumerStatefulWidget` for slider drag-follow local state.
...
## Interfaces
- Consumes: `appSettingControllerProvider` (Task 1), `SettingsSectionScaffold` + `accentSwatches` + `segmentTapTargetStyle` (Task 2), `AppStrings`
- Produces: `WatermarkDefaultsSectionScreen` widget (used by Task 11 routes)

## Pre-applied corrections (from Task 3/4 findings)
1. **`valueOrNull` → `.value`**: Riverpod 3.3.2 lacks `valueOrNull`. Use `asyncSettings.value` (equivalent — returns `T?`, null on loading/error).
2. **Dual import in test**: test references `AppDatabase` directly, so needs BOTH `package:sitemark/app.dart` (for `databaseProvider`) AND `package:sitemark/data/app_database.dart` (for `AppDatabase` type).
3. **`copyWith` for watermark fields — NO `Value(...)` needed**: all 4 watermark fields (`defaultWatermarkPosition`, `defaultWatermarkOpacity`, `defaultWatermarkFontScale`, `defaultWatermarkAccentColorArgb`) are NON-nullable in the drift schema (`lib/data/app_database.dart:45-51`). Drift generates `copyWith` with plain `T?` params for non-nullable columns. So `copyWith(defaultWatermarkOpacity: 0.95)` compiles fine. (Only `localeCode` is nullable and needed `Value(...)` — that was Task 4.)
...
- All l10n keys verified in `global_settings_screen.dart` lines 332–498: `newProjectDefaults`, `watermarkPosition`, `bottomLeft`, `bottomRight`, `watermarkOpacity`, `opacityHint`, `watermarkFontSize`, `fontScaleHint`, `accentColor`, `green`, `blue`, `orange`.
- All 4 watermark fields are NON-nullable (`lib/data/app_database.dart:45-51`): `defaultWatermarkPosition` (text, non-null), `defaultWatermarkOpacity` (real, non-null), `defaultWatermarkFontScale` (real, non-null), `defaultWatermarkAccentColorArgb` (int, non-null). Plain `copyWith(field: value)` compiles — NO `Value(...)` wrapper needed.
- The `_dragValue` / `_fontScaleDragValue` local state + `Builder` pattern is migrated verbatim from `global_settings_screen.dart:366-468`. The only change: `_apply((db) => db.updateAppSettings(...))` → `ref.read(appSettingControllerProvider.notifier).update((s) => s.copyWith(...))`.
- `_AccentChoice` is migrated verbatim from `global_settings_screen.dart:710-742` (the existing private widget). The new file recreates it as a file-private `_AccentChoice`.
- `accentSwatches` is imported from `settings_section_scaffold.dart` (Task 2's public const).
- The existing `global_settings_screen_test.dart` has identical assertions for these 4 controls (opacity/font-scale/accent/position). The new test mirrors them. The old assertions will be removed in Task 10/12.
