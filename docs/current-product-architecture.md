# SiteMark 当前产品边界与总体架构

> 状态：当前设计
> 适用版本：v0.2.0
> 本文描述已经实现并发布的行为；未来计划以 README 的 P0/P1 路线图为准。

## 1. 产品定位

SiteMark（工程印记）是面向工程现场记录的 Android 水印相机。应用负责项目、
现场信息、记录管理和水印处理，实际拍照交给手机系统或厂商相机完成。

当前产品边界：

- Android 12（API 31）及以上；界面支持简体中文和英文；
- 无广告、无账号、无云端、无统计分析，不申请网络权限；
- 使用系统相机，不在应用内实现或集成第三方相机界面；
- 项目数据库、私有原图和处理中间文件保存在本机；
- 水印成片发布到系统相册 `Pictures/SiteMark`；
- 支持项目导出和所选记录导出，但当前不支持将 ZIP 导回应用恢复数据库；
- 不支持 iOS、多人协作、云同步、图库导入和自由拖拽式水印模板。

## 2. 用户主流程

1. 新建项目，必要时调整该项目的水印位置、透明度、字体比例和强调色；
2. 填写工程部位、工作内容、拍摄人和可选备注；
3. 选择是否允许前台定位，然后调用系统相机；
4. 系统相机返回后立即生成记录，定位解析、水印渲染和相册发布在后台继续；
5. 用户可以保留上一张的工程部位、工作内容和拍摄人继续拍摄，备注会清空；
6. 在项目记录或全部记录中搜索、筛选、预览、导出、再次保存、清理原图或删除；
7. 卸载或换机前，将重要记录和原图导出到应用私有目录之外进行外部归档。

## 3. 总体架构

| 层 | 技术 | 主要职责 |
| --- | --- | --- |
| Flutter 应用层 | Flutter、Material 3、Riverpod、GoRouter | 页面、导航、中英文、主题、表单状态、记录交互 |
| 数据层 | Drift、SQLite | 项目、设置、拍摄记录、状态流转、筛选与迁移 |
| 后台任务层 | WorkManager、Dart 后台 isolate | 串行任务、失败重试、启动与重启恢复 |
| Android 集成层 | Kotlin、Pigeon、FlutterPlugin、ActivityAware | 系统相机、ContentProvider、EXIF 检查、前台定位、MediaStore |
| 图像核心层 | Rust、flutter_rust_bridge | EXIF 方向、SHA-256、全分辨率水印、CSV/JSON/ZIP 导出 |

Flutter 与 Rust 之间只传文件路径和结构化参数，不把整张全分辨率图片作为 Dart
字节数组跨 FFI 传递。Android 平台能力集中在仓库内插件
`packages/sitemark_system_api`，前台 Activity 和后台 FlutterEngine 共用同一接口。

## 4. 数据模型

当前 Drift schema 为版本 4，核心实体如下：

- `projects`：项目名称、说明以及项目级水印设置；
- `captures`：现场字段、照片编号、时间、定位、路径、哈希、处理状态和原图清理状态；
- `app_settings`：主题、语言以及新建项目使用的默认水印参数。

`captures.project_id` 通过外键关联项目，并定义数据库级级联删除。照片编号在系统相机成功返回后分配，
格式包含安全化项目名称、项目 ID、日期和当日序号，以避免重名项目产生文件冲突。
照片编号用于数据库、文件名、CSV 和 JSON，不绘制在水印画面中。

## 5. 权限与安全边界

发布 APK 可申请前台粗略/精确定位；拒绝定位不阻止拍照。WorkManager 使用唤醒、
开机恢复、前台服务和 Android 13+ 通知能力。应用不申请：

- `CAMERA`；
- `INTERNET`、`ACCESS_NETWORK_STATE`；
- 后台定位；
- 广泛媒体访问或传统外部存储权限。

GitHub 仓库链接由外部浏览器打开，SiteMark 本身不联网。SHA-256 用于本地完整性
检查和记录追溯，不代表司法鉴定、可信时间戳或第三方存证。

## 6. 数据保留边界

应用私有数据库和原图会在卸载时被 Android 删除；已经发布到
`Pictures/SiteMark` 的水印照片通常仍会保留。导出 ZIP 是外部归档格式，
当前版本没有导入或数据库恢复入口，因此不能把“已导出”等同于“可恢复应用状态”。

## 7. 当前质量基线

v0.2.0 发布时的自动化记录为：185 项 Flutter 测试、20 项 Rust 测试，
`flutter analyze`、Rust fmt/Clippy 和 Android 单元测试通过。不同厂商系统相机的
启动速度、返回行为和后台限制仍需通过 README P0 真机兼容性矩阵持续补充。

## 8. 文档维护规则

本文件只记录当前已经实现的产品边界和架构。阶段性任务、PR 修复过程和被替代的
假设留在 `docs/superpowers/` 作为历史，不再从 README 的当前设计入口展示。
任何改变权限、数据生命周期、拍摄状态机、导出可恢复性或技术分层的修改，必须同步
更新本文件及[关键技术决策记录](decision-records.md)。
