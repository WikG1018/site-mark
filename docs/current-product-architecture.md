...
同一项目内名称按规范化键唯一。模板从数据类型到界面、归档均不包含备注。模板文本长度
以 Unicode 标量计数：Dart 使用 `runes.length`，Rust 使用 `chars().count()`，SQLite
使用 `length()` 的同时以 `instr(..., char(0)) = 0` 明确拒绝 U+0000，避免 NUL 截断绕过长度约束。
...
schema v4 支持空白项目、项目说明、创建时间、水印设置、项目内模板、备份快照和遗漏
计数，并兼容 v1/v2/v3；旧归档恢复为空模板列表。恢复项目、记录和模板由同一恢复
所有权令牌约束，照片先进入私有暂存区。提交前失败会回滚数据库、暂存文件和已规划的
目标文件；提交标记已写入但收尾中断时，由下次启动完成所有权清理。拍摄记录编辑页生成
的普通分享 ZIP 仍然不是备份，不能用于恢复。
...
v0.9.0 候选必须通过 Flutter 全量测试与静态分析、Rust fmt/Clippy/全量测试、Android
插件单元测试和 Debug/Release APK 构建。备份或恢复失败只记录固定错误分类、数量和
耗时等允许字段，不持久化项目名称、模板内容、路径、SQL、原始异常或堆栈。
