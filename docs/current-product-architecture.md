...
ZIP 使用项目归档 schema v4，保存空白项目、项目说明、创建时间、水印设置、项目内模板、
快照和遗漏计数，并兼容 v1/v2/v3 旧项目 ZIP；多项目外层 bundle schema 仍为 v1。
旧项目 ZIP 恢复为空模板列表。
...
插件单元测试和 Debug/Release APK 构建。当前持久化诊断只记录备份操作的固定结果分类、
数量和耗时，不记录恢复结果，也不持久化项目名称、模板内容、路径、SQL、原始异常或堆栈。
