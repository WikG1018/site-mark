...
| 当前版本 | 鸿蒙原生见 `ohos-native/AppScope/app.json5`（`1.0.13`）；Android 见 `pubspec.yaml`（`1.0.24+39`） |
...
- **`v1.0.24` 是 Latest**（全屏预览重影修复：Hero 按入口照片 ID 跟随，相邻预取推迟到转场结束）；`native-v1.0.13` 为当前鸿蒙原生版本（未签名 HAP）。后续发版按清单完成拍照/后台与备份恢复**真机**回归，并覆盖有代表性的厂商相机（小米/OPPO/vivo/三星/Pixel 等）。
