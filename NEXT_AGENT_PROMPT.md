...
| 当前版本 | 鸿蒙原生见 `ohos-native/AppScope/app.json5`（`1.0.11`）；Android 见 `pubspec.yaml`（`1.0.19+34`） |
...
- **`v1.0.19` 是 Latest**（滚动藏栏动画放慢到 280ms；项目列表去掉多余顶距；滚动时顶栏不再变深）；`native-v1.0.11` 为当前鸿蒙原生版本（未签名 HAP）。后续发版按清单完成拍照/后台与备份恢复**真机**回归，并覆盖有代表性的厂商相机（小米/OPPO/vivo/三星/Pixel 等）。
