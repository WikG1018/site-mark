...
| 当前版本 | 鸿蒙原生见 `ohos-native/AppScope/app.json5`（`1.0.4`）；Android 见 `pubspec.yaml`（`1.0.11+26`） |
...
- **`v1.0.11` 是 Latest**（targetSdk 37，已真机回归）；`native-v1.0.4` 为当前 Pre-release，转 Latest 前按清单完成拍照/后台与备份恢复**真机**回归，并覆盖有代表性的厂商相机（小米/OPPO/vivo/三星/Pixel 等）。  
