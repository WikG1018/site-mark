...
| 项 | 当前值 |
| --- | --- |
| 产品 | SiteMark（工程印记）：离线优先的工程水印相机（Android 稳定 + HarmonyOS NEXT 原生开发中），可选 NAS 同步（WebDAV/SFTP/SMB，默认关闭） |
...
| 当前版本 | 鸿蒙原生见 `ohos-native/AppScope/app.json5`（`1.0.13`）；Android 见 `pubspec.yaml`（`1.0.23+38`） |
...
- **`v1.0.23` 是 Latest**（安卓动效深度修复：转场层序、照片详情冻结 secondary、Tab 非对称 FadeThrough、列表预取 500px、藏栏阈值 16px；iOS 导航 chrome/插件修复）；`native-v1.0.13` 为当前鸿蒙原生版本（未签名 HAP）。后续发版按清单完成拍照/后台与备份恢复**真机**回归，并覆盖有代表性的厂商相机（小米/OPPO/vivo/三星/Pixel 等）。
