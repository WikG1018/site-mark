...
| 当前版本 | 鸿蒙原生见 `ohos-native/AppScope/app.json5`（`1.0.8`）；Android 见 `pubspec.yaml`（`1.0.16+31`） |
...
- **`v1.0.16` 是 Latest**（NAS 设置页排版优化，Android-only；功能基线为 v1.0.15 的审查修复 + v1.0.14 的三端 NAS 同步，D-023 修订）；`native-v1.0.8` 为当前鸿蒙原生版本（未签名 HAP）。后续发版按清单完成拍照/后台与备份恢复**真机**回归，并覆盖有代表性的厂商相机（小米/OPPO/vivo/三星/Pixel 等）。
