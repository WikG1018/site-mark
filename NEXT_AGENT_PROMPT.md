...
| 当前版本 | 鸿蒙原生见 `ohos-native/AppScope/app.json5`（`1.0.12`）；Android 见 `pubspec.yaml`（`1.0.20+35`） |
...
- **`v1.0.20` 是 Latest**（安卓流畅度：列表卡片与页面停用实时毛玻璃、导航 dock 安卓保留真实模糊，页面转场改单层轻量滑动；项目详情顶距修正）；`native-v1.0.12` 为当前鸿蒙原生版本（未签名 HAP）。后续发版按清单完成拍照/后台与备份恢复**真机**回归，并覆盖有代表性的厂商相机（小米/OPPO/vivo/三星/Pixel 等）。
