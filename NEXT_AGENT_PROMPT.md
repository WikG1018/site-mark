...
| 当前版本 | 鸿蒙原生见 `ohos-native/AppScope/app.json5`（`1.0.13`）；Android 见 `pubspec.yaml`（`1.0.28+43`） |
...
- **`v1.0.28` 是 Latest**（澎湃OS 风格动效/材质升级 + MiSans 子集）；`native-v1.0.13` 为当前鸿蒙原生版本（未签名 HAP）。后续发版按清单完成拍照/后台与备份恢复**真机**回归，并覆盖有代表性的厂商相机（小米/OPPO/vivo/三星/Pixel 等）。
