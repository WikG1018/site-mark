...
[![Latest](https://img.shields.io/badge/latest-v1.0.25-176B55)](https://github.com/WikG1018/site-mark/releases/tag/v1.0.25)
...
**Current stable version (Latest): [`v1.0.25`](https://github.com/WikG1018/site-mark/releases/tag/v1.0.25)**
...
Requires Android 12 (API 31) or later. `v1.0.25` is set as Latest: first-level settings toggles (save to gallery / location / completion notifications); selection export as project folders of photos only; glass capsule toasts without the dead View action; glass FABs on all platforms with a scroll-stable capture button. The feature baseline remains the optional three-protocol NAS sync (WebDAV/SFTP/SMB, off by default, passwords in system secure storage). Back up important projects regularly — including the private original photos — and copy the backup files outside the app's directories.
...
| --- | --- | --- |
| arm64 | Recommended; almost all recent Android phones | [sitemark-v1.0.25-arm64.apk](https://github.com/WikG1018/site-mark/releases/download/v1.0.25/sitemark-v1.0.25-arm64.apk) |
| universal | Use when the processor architecture is unknown or arm64 cannot be installed; larger file | [sitemark-v1.0.25-universal.apk](https://github.com/WikG1018/site-mark/releases/download/v1.0.25/sitemark-v1.0.25-universal.apk) |
| SHA-256 | Verify the integrity of downloaded files | [SHA256SUMS.txt](https://github.com/WikG1018/site-mark/releases/download/v1.0.25/SHA256SUMS.txt) |
...
- **v1.0.25** (Latest): first-level settings toggles (save to gallery / location / completion notifications); selection export as project folders of photos only; glass capsule toasts without the dead View action; glass FABs on all platforms with a scroll-stable capture button.
- **v1.0.24** (Latest): fixes the ghosted fullscreen entry animation when opening the viewer from any record other than the first — the fullscreen Hero pinned to the initial index, so prepending adjacent records moved the tag onto the wrong photo while the real entry photo rendered in place under the flying shuttle; the hero now follows the entry photo by ID, and adjacent prefetch waits for the transition to finish.
- **v1.0.23**: Android motion depth fixes (scale outside RepaintBoundary, frozen photo-detail secondary, asymmetric tab FadeThrough, 500px list prefetch, 16px hide threshold, 96MB image cache); iOS native nav chrome / edge-swipe back and plugin lifetime fixes.
