...
class AppStrings {
  ...
  String get privacyStatements => _english
      ? 'No ads · No account · No third-party cloud · Optional NAS sync · System camera'
  ...
  String get privacySummary => _english
      ? 'Offline by default. The only network surface is optional NAS sync (WebDAV, SFTP or SMB) that you enable yourself; it connects only to the server you configure. GitHub links open in an external browser. Foreground location is used only when requested, and a diagnostic bundle reaches the system share sheet only after confirmation.'
      : '默认离线。唯一的网络出口是你主动配置并启用的 NAS 同步（WebDAV/SFTP/SMB），只连接你自己的服务器。GitHub 链接交给外部浏览器。前台定位仅在用户主动请求时使用，诊断包仅在用户确认后交给系统分享面板。';
  ...
  String get nasRootPathHint => _english ? 'e.g. /SiteMark' : '例如 /SiteMark';
  ...
  String get nasAcceptInvalidTls =>
      _english ? 'Accept self-signed certificates' : '接受自签名证书';
  ...
  String get nasFingerprintTitle =>
      _english ? 'Verify server fingerprint' : '验证服务器指纹';
  ...
  String get nasErrorConnectionFailed => _english
      ...
      : '无法连接服务器，请检查地址和网络。';
  String get nasErrorAuthFailed =>
      _english ? 'Incorrect username or password' : '用户名或密码不正确';
  String get nasErrorTimeout => _english ? 'Connection timed out' : '连接超时';
  String get nasErrorTlsError =>
      _english ? 'TLS or certificate error' : 'TLS 或证书错误';
  ...
  String get nasErrorHostKeyChanged => _english
      ...
      : '服务器指纹已变化，请重新测试连接并确认。';
  String get nasErrorProtocolError =>
      _english ? 'The server returned an unexpected response' : '服务器响应异常';
  String get nasErrorQuotaInsufficient =>
      _english ? 'The NAS is out of storage space' : 'NAS 存储空间不足';
  String get nasErrorPathInvalid => _english ? 'Invalid remote path' : '远程路径无效';
  ...
  String get nasSyncDirectionHelp => _english
      ? 'Upload only: watermarked photos are copied to the NAS and never pulled back.\n'
            'Two-way: import NAS photos missing on this phone as new records; '
            'deleting a record here also deletes its NAS copy.'
  ...
  String get nasUsageBody => _english
      ? '• Photos upload to {root}/{project}/{photo number}.jpg. Folders are created automatically.\n'
            ...
            '• Two-way: import NAS-only photos; deleting a record removes its NAS copy.'
      : '• 照片上传到 {根目录}/{项目}/{照片编号}.jpg，目录会自动创建。\n'
            ...
            '• 双向：可导入 NAS 上本机没有的照片；删除记录会删 NAS 副本。';
