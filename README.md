...
- 鸿蒙发布相册：无 ACL 时 `publishJpeg` 走 `showAssetsCreationDialog`，不再落到文档选择器；`detectGalleryAccess` 无 ACL 证明不得返回 `acl`；无相册 dump
