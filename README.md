...
- 鸿蒙 CI：`ohos.yml` 已纳入 widget / lifecycle / 备份导入（`project_import` / `project_export` / 备份设置页）与拍摄失败引导。取消拍照不占下一张当日编号。无拍成 dump，不得写备份已进系统文件管理
