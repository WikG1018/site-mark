...
| 范围 | 当前状态 |
| --- | --- |
| 一级导航 | “项目 / 全部记录 / 设置”通过覆盖在内容上方的悬浮 Dock 切换，并分别保留列表、搜索和筛选状态；内容可从 Dock 下方连续滚动，进入详情等二级页面后 Dock 隐藏 |
...
| 批量操作 | 多选时以同位置的紧凑悬浮 Dock 替换一级导航，保持照片浏览空间；支持按当前筛选结果全选/取消全选、导出、再次保存、清理原图和删除整条记录 |
...
| 体验 | 玻璃材质导航与卡片、稳定且符合层级关系的页面转场、图片 Hero 动画、返回逻辑、减少动画适配，以及可继续加载的记录和全屏图片列表 |
