...
![Status](https://img.shields.io/badge/status-v0.5.3--prerelease-orange)

**当前版本：[`v0.5.3` 签名预发布版](https://github.com/WikG1018/site-mark/releases/tag/v0.5.3)**
...
| 安装包 | 适用设备 | 下载 |
| --- | --- | --- |
| arm64 | 推荐；绝大多数近年 Android 手机 | [下载 sitemark-v0.5.3-arm64.apk](https://github.com/WikG1018/site-mark/releases/download/v0.5.3/sitemark-v0.5.3-arm64.apk) |
| universal | 不确定处理器架构或 arm64 无法安装时使用；文件更大 | [下载 sitemark-v0.5.3-universal.apk](https://github.com/WikG1018/site-mark/releases/download/v0.5.3/sitemark-v0.5.3-universal.apk) |
| SHA-256 | 校验下载文件完整性 | [查看 SHA256SUMS.txt](https://github.com/WikG1018/site-mark/releases/download/v0.5.3/SHA256SUMS.txt) |

> **重要数据警告：** 卸载 SiteMark 会删除项目数据库和应用私有原图；已经发布到 `Pictures/SiteMark` 的水印照片通常仍会保留。卸载或切换签名前，请先导出重要项目（建议勾选“包含私有原图”）；导出的单项目 ZIP 可以在重装或换机后从首页“导入项目备份”恢复到应用内。
...
5. 遇到签名冲突时，先将重要项目和原图导出为 ZIP 外部归档，卸载旧版后重新安装，再从首页“导入项目备份”恢复。
6. [Release 页面](https://github.com/WikG1018/site-mark/releases/tag/v0.5.3)提供版本说明和校验文件。
...
| 备份恢复 | 单项目导出 ZIP 可从首页重新导入：恢复项目、记录、编号、时间和（可选）原图，原图 SHA-256 会重新校验 |
...
8. 卸载或换机前将重要项目导出为 ZIP（勾选“包含私有原图”即为完整备份）；之后可在首页“导入项目备份”恢复。
...
但不会显示照片编号。新照片文件名采用
`{安全化项目名称}-SM-{yyyyMMdd}-{全应用当日序号}.jpg`，例如
`云湖之城-SM-20260717-003.jpg`。旧照片不会自动改名，原编号仍保存在数据库记录中。

新建项目会忽略名称首尾空白、连续空白和英文大小写检查重名；如果两个名称生成相同的
安全文件名（例如 `A/B` 与 `A:B`），也会拒绝创建，避免后续照片文件冲突。

项目导出为 ZIP，可包含：

- 已完成的水印 JPEG；
- 带 UTF-8 BOM 的 CSV，方便在常见表格软件中直接打开；
- 带版本号的 JSON manifest（v2 起额外记录项目水印设置和定位数据，用于恢复）；
- CSV 与 JSON manifest 均保留照片编号；
- 用户明确选择时附带的私有原图。

单项目导出 ZIP 同时是备份文件：从首页“导入项目备份”选择该 ZIP，即可恢复为新项目。
恢复会保留照片编号、拍摄时间、工程信息和原图 SHA-256（导入时重新校验，不一致即中止），
v2 备份还会还原项目水印设置和 GPS 坐标；早期 v1 导出包同样可以导入，水印设置使用默认值。
跨项目“多选导出”ZIP 用于对外分享，暂不支持导入恢复。导入的记录未发布到系统相册，
可通过现有的“再次保存”批量操作重新发布。
...
并在 [Release 页面](https://github.com/WikG1018/site-mark/releases/tag/v0.5.3)查看版本说明和校验文件。
