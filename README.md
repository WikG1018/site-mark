...
| 未完成 | 签名 release、相机拍成 dump / ACL / `ohos-arm64` 对等、系统相册 dump 未证、系统通知未证、系统分享未证、系统文件选择恢复未证、系统外链未证、真机回归 |
| 引擎 | `tool/ohos/engine_status.md`：**degraded** |

**交接文档（换 Agent 接手先读）：** [docs/superpowers/handoffs/2026-08-20-ohos-agent-handoff.md](docs/superpowers/handoffs/2026-08-20-ohos-agent-handoff.md) —— 自包含的现状、硬红线、诚实边界、测试/CI 方法、云端与本地环境分工和下一步建议；只依赖本仓库与 GitHub，不依赖任何本地机器路径。

后续实施计划：[2026-08-21-harmonyos-share-cancel-no-failure.md](docs/superpowers/plans/2026-08-21-harmonyos-share-cancel-no-failure.md)（Task 60：ShareKit 分享面板用户取消不报失败；空源 / `ohos_not_ready` / 其它真实错误仍失败。官方测试闭环后仍无分享面板 dump，不得写系统分享已通）。前序：[2026-08-20-harmonyos-album-cancel-sandbox.md](docs/superpowers/plans/2026-08-20-harmonyos-album-cancel-sandbox.md)（Task 59：相册保存对话框取消 / 空 destinations / 其它相册错误回退沙箱，不把拍摄打成 failed。官方测试已绿，无相册 dump，队列仍是应用内内存串行）。前序：[2026-08-20-harmonyos-empty-camera-no-photo-number.md](docs/superpowers/plans/2026-08-20-harmonyos-empty-camera-no-photo-number.md)（Task 58：pending 空文件恢复不占下一张当日编号；live `launchCamera` 空 URI / materialize 无内容走 cancelled，不再留下失败行。权限拒绝 / 无 Ability / picker 抛错 / 拷贝失败仍 failed。官方测试已绿，无拍成 dump，队列仍是应用内内存串行）。前序：[2026-08-20-harmonyos-denied-location-burst-numbers.md](docs/superpowers/plans/2026-08-20-harmonyos-denied-location-burst-numbers.md)（Task 57：拒绝定位仍 queued 出片且有当日编号；连拍当日 `001` 然后 `002`；`ohos.yml` 纳入 `capture_location_coordinator_test`。生产路径已先 `markCaptured` 再解析定位，本刀只补断言与 CI。官方测试已绿，无拍成 dump，队列仍是应用内内存串行）。前序：[2026-08-20-harmonyos-cancel-no-photo-number.md](docs/superpowers/plans/2026-08-20-harmonyos-cancel-no-photo-number.md)（Task 56：取消拍照不占下一张当日编号；`ohos.yml` 纳入 `capture_failure_guidance_test`。生产路径已删 pending 且不 `markCaptured`，本刀只补断言与 CI。官方测试已绿，无拍成 dump，队列仍是应用内内存串行）。前序：[2026-08-20-harmonyos-ci-widget-lifecycle-backup.md](docs/superpowers/plans/2026-08-20-harmonyos-ci-widget-lifecycle-backup.md)（Task 55：`ohos.yml` 纳入 `widget_test` / `app_lifecycle_test` / `project_import_test` / `project_export_test` / `backup_restore_section_screen_test`。不改生产语义。官方测试已绿，无拍成 dump，队列仍是应用内内存串行）。前序：[2026-08-20-harmonyos-capture-workflow-ci-delayed.md](docs/superpowers/plans/2026-08-20-harmonyos-capture-workflow-ci-delayed.md)（Task 54：`ohos.yml` 纳入 `capture_workflow_test`；拍摄页入队失败 delayed 后仍可继续拍。生产 delayed 已有，本刀只补 CI 与 widget 锁。官方测试已绿，无拍成 dump，队列仍是应用内内存串行）。前序：[2026-08-20-harmonyos-kill-process-four-windows.md](docs/superpowers/plans/2026-08-20-harmonyos-kill-process-four-windows.md)（Task 53：启动恢复四窗各自可重入、互不阻塞。`AppStartupRecovery` 是回调式：先串行清理中断导入，再并行开工相册待清理 / 发布日记 / 相机 / 定位 / 队列。日记抽出为独立 `recoverPublishJournals()`。官方测试已绿，无杀进程 dump，队列仍是应用内内存串行）。前序：[2026-08-20-harmonyos-ci-and-captureid-delete.md](docs/superpowers/plans/2026-08-20-harmonyos-ci-and-captureid-delete.md)（Tasks 51–52：`ohos.yml` 纳入 UI / 降级引擎 / GPS 测试；相册适配器删除按 URI 方案路由，备份恢复同编号不得串 URI。官方测试已绿，无相册 dump 不得写系统相册已通）。前序：[2026-08-20-harmonyos-dynamic-color-honesty.md](docs/superpowers/plans/2026-08-20-harmonyos-dynamic-color-honesty.md)（Task 50：鸿蒙外观页隐藏「跟随系统取色」空开关，始终露出应用主题色，并标明「鸿蒙暂不支持壁纸动态取色」。官方测试已绿，不得写动态取色已对等 Android Material You）。前序：[2026-08-20-harmonyos-capture-degraded-watermark-hint.md](docs/superpowers/plans/2026-08-20-harmonyos-capture-degraded-watermark-hint.md)（Task 49：拍摄页在 `imagePipelineProvider.isDegraded` 时显示与存储页/详情页相同的「降级水印引擎」提示。官方测试已绿，无拍成 dump 不得写水印已对等 Android 像素）。再前：[2026-08-20-harmonyos-gallery-access-honesty.md](docs/superpowers/plans/2026-08-20-harmonyos-gallery-access-honesty.md)（Task 48：无 ACL 证明时 `detectGalleryAccess` 返回 `pickerFallback`，存储页和拍摄页显示「未进入系统相册」；发布仍可尝试 `createAsset`，媒体权限不得当成 ACL。官方测试已绿，无相册 dump 不得写系统相册已通）。再前：[2026-08-20-harmonyos-jpeg-gps-fallback.md](docs/superpowers/plans/2026-08-20-harmonyos-jpeg-gps-fallback.md)（Task 47：宿主 `inspectImage` GPS 空时 Dart 解析 JPEG EXIF）。再前：[2026-08-20-harmonyos-queue-retry.md](docs/superpowers/plans/2026-08-20-harmonyos-queue-retry.md)（Task 46：应用内串行队列对 `retry` / runner 抛错做 30s 起指数退避，不挡后续 capture；官方测试已绿，无拍成 dump 不得写后台渲染已对等 Android WorkManager）。再前：[2026-08-20-harmonyos-degraded-exif-bake.md](docs/superpowers/plans/2026-08-20-harmonyos-degraded-exif-bake.md)（Task 45：降级 `render` 叠水印前烘焙 EXIF orientation，竖拍 6 号方向 48×32 → 32×48；官方测试已绿，无拍成 dump 不得写方向已对等）。再前：[2026-08-20-harmonyos-degraded-watermark.md](docs/superpowers/plans/2026-08-20-harmonyos-degraded-watermark.md)（Tasks 43–44：降级 `render` 用 `dart:ui` 半透明卡片叠与 Rust `labels()` 同字段的中文/英文水印，不再只画 `SiteMark`；官方测试已绿，无拍成 dump 不得写水印已对等 Android 像素）。再前：[2026-08-20-harmonyos-album-save-dialog.md](docs/superpowers/plans/2026-08-20-harmonyos-album-save-dialog.md)（Tasks 41–42：无 ACL 发布走 `showAssetsCreationDialog` 系统相册保存对话框，不再落到文档选择器；通道测试已绿，无相册 dump 不得写系统相册已通）。再前：[2026-08-19-harmonyos-capture-sandbox-copy.md](docs/superpowers/plans/2026-08-19-harmonyos-capture-sandbox-copy.md)（Tasks 39–40：`CameraPicker` 省略沙箱 `saveUri`，`file://media/` 用 `fs.openSync(uri)` 拷进 `files/originals`；通道测试已绿，无拍成 dump 不得写相机已拍成）。再前：[2026-08-19-harmonyos-open-link.md](docs/superpowers/plans/2026-08-19-harmonyos-open-link.md)（Tasks 37–38：关于页 GitHub 走 `startAbility` 隐式 Want；`main.dart` 鸿蒙外链不再覆盖成 no-op；通道测试已绿，无浏览器 dump 不得写系统外链已通）。再前：[2026-08-19-harmonyos-notifications.md](docs/superpowers/plans/2026-08-19-harmonyos-notifications.md)（Tasks 35–36：NotificationKit）。再前：[2026-08-19-harmonyos-share-file.md](docs/superpowers/plans/2026-08-19-harmonyos-share-file.md)（Tasks 33–34：ShareKit）。再前：[2026-08-19-harmonyos-inspect-image.md](docs/superpowers/plans/2026-08-19-harmonyos-inspect-image.md)（Tasks 31–32：拍成后读图走 ImageKit `inspectImage`）。再前：[2026-08-19-harmonyos-pick-archive.md](docs/superpowers/plans/2026-08-19-harmonyos-pick-archive.md)（Tasks 29–30：鸿蒙恢复选文件走原生 `DocumentViewPicker`）。再前：[2026-08-19-harmonyos-restore-import.md](docs/superpowers/plans/2026-08-19-harmonyos-restore-import.md)（Tasks 25–28：降级读档）。再前：[2026-08-19-harmonyos-save-archive.md](docs/superpowers/plans/2026-08-19-harmonyos-save-archive.md)（Tasks 21–24：沙箱 schema 5 zip + picker 弹出）。再前：[2026-08-19-harmonyos-records-backup.md](docs/superpowers/plans/2026-08-19-harmonyos-records-backup.md)。再前：[2026-08-19-harmonyos-capture-path.md](docs/superpowers/plans/2026-08-19-harmonyos-capture-path.md)。再前：[2026-08-18-harmonyos-product-runtime.md](docs/superpowers/plans/2026-08-18-harmonyos-product-runtime.md)。全量 HAP 编译记录见 [2026-08-18-harmonyos-full-product-hap.md](docs/superpowers/plans/2026-08-18-harmonyos-full-product-hap.md)。前期 Tasks 0–5 见 [2026-08-17-harmonyos-next-adaptation.md](docs/superpowers/plans/2026-08-17-harmonyos-next-adaptation.md)。规格见 [harmonyos-next-adaptation-design.md](docs/superpowers/specs/2026-08-17-harmonyos-next-adaptation-design.md)。
...
- `OhosPlatformServices`、应用内串行队列（`retry` / 抛错 30s 起指数退避，最多 3 次调度重试；不是 WorkManager / WorkScheduler 进程保活）、按 `captureId` 的发布日记
- ACL 相册 + picker / 沙箱托底（发布仍可尝试 `createAsset`；探测无 ACL 证明时返回 `pickerFallback`，存储页和拍摄页显示「未进入系统相册」。模拟器未证明 ACL）
...
- 鸿蒙分享：`OhosShareFileService` + 宿主 `shareFile`（ShareKit `ShareController`，zip/jpeg/png 走对应 UTD）；产品入口不再覆盖成 no-op；分享面板用户取消不报失败；无分享面板 dump
