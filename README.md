...
| 项目管理 | 支持重命名和删除项目；重命名不改历史照片，删除项目不动系统相册 |
...
1. 新建项目并设置水印。
2. 填写工程部位、工作内容和拍摄人。
3. 需要位置时点击定位提示并决定是否授权；只有这次主动操作会请求权限，拒绝后仍可拍照。
4. 调用系统相机完成拍摄。
5. 上一张在后台处理时继续拍摄。
...
- **删除项目**：删除应用内项目数据库、私有原图和水印文件，但不会删除系统相册照片或已经导出的备份文件；项目删除界面不提供删除系统相册照片的选项。
...
| 权限 | 用途 |
| --- | --- |
| `ACCESS_COARSE_LOCATION`、`ACCESS_FINE_LOCATION` | 可选的前台定位；仅在用户点击定位提示时请求，拒绝后仍可拍照 |
...
项目重命名只影响项目列表显示以及重命名后新拍照片的项目名称；历史照片编号、文件路径、
文件名和水印证据均保持不变。

在“设置 → 备份与恢复”中可以选择一个或多个项目，并决定是否包含应用私有原图。
包含原图的备份更完整，但文件会明显增大。备份内容包括：
...
只选择一个项目时会生成仍可单独恢复的单项目 ZIP；选择多个项目时会生成一个可恢复的
