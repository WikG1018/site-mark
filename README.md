...
- **备份格式升级至 schema v4**：模板随所属项目备份和恢复，继续兼容 v1、v2、v3 项目备份。
...
单项目备份包含项目元数据、项目水印设置、项目内命名模板、已完成的水印 JPEG、UTF-8 BOM CSV 和 JSON manifest；用户选择时还会包含私有原图。schema v4 继续记录备份快照时间和被明确跳过的失败记录数量，并新增模板数组。

### 恢复

- 支持当前 schema v4，以及旧版 v1/v2/v3 项目备份；旧备份恢复后模板列表为空；
