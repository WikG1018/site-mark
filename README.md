...
- **项目 ZIP 升级至 schema v4**：模板随所属项目备份和恢复，继续兼容 v1、v2、v3 旧项目 ZIP；多项目备份的外层 bundle 仍为 schema v1。
- **恢复一致性**：恢复前校验模板数量、唯一性、字段长度和非法字符；失败时按项目所有权和文件暂存记录回滚，不留下半恢复项目。

## 从早期版本累计完成的改进

| 范围 | 当前状态 |
| --- | --- |
| 拍摄 | 调用系统/厂商相机，连续拍摄时后台生成水印；支持最近字段建议和项目内命名模板 |
...
每个项目 ZIP 包含项目元数据、项目水印设置、项目内命名模板、已完成的水印 JPEG、UTF-8 BOM CSV 和 JSON manifest；用户选择时还会包含私有原图。项目 ZIP schema v4 继续记录备份快照时间和被明确跳过的失败记录数量，并新增模板数组。多项目外层 bundle 只负责组织这些项目 ZIP，其 schema v1 未改变。

### 恢复

- 项目 ZIP 支持当前 schema v4，以及旧版 v1/v2/v3；这里的兼容版本不指多项目外层 bundle，旧项目 ZIP 恢复后模板列表为空；
