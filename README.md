...
- 鸿蒙 CI：`ohos.yml` 已纳入 widget / lifecycle / 备份导入（`project_import` / `project_export` / 备份设置页）、拍摄失败引导与定位协调器。取消拍照不占下一张当日编号；拒绝定位仍 queued 出片；连拍当日 `001` 然后 `002`。无拍成 dump，不得写备份已进系统文件管理
