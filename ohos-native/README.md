...
- 动效统一由 `MotionPolicy` 驱动并遵循系统减少动画：跨页面 Hero 使用 Navigation geometry 共享转场（列表卡片→详情→查看器），查看器双击缩放、单击还原和 pinch 吸附走缓动过渡，条件面板（筛选、模板、建议、删除确认、消息）使用统一的淡入淡出加位移转场，全屏翻页使用查看器专用时长。
