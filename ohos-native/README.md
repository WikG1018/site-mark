...
- ArkTS 全量测试、鸿蒙数据库契约测试、共享 Rust 核心与 Flutter 回归门禁均已通过；产品文档不固定测试数量，日期、命令和本次计数见[最新验证记录](docs/verification-2026-09-26.md)。
...
- 动效与材质按 D-024 对齐 Android（决策源 `../docs/motion-and-material-spec.md`）：时长/弹簧/曲线统一取自 `MotionTokens`，六档圆角与玻璃、悬浮阴影走 `UiTokens` 单配方；条件面板与弹窗使用弹簧进/加速出转场，消息行按 toast 语义上升入场；按钮与 Dock 有按压缩放（0.96）与语义触感（切换 selectionClick、保存成功 lightImpact、进入多选 mediumImpact、破坏性确认 heavyImpact；导航与校验失败静音）；列表行入场每行一次且首屏不交错；看图带橡皮筋、惯性收尾与弹簧回位。全部动效经 `MotionPolicy` 折叠系统减少动画。
...
| NAS 同步 | WebDAV/SFTP/SMB 可选同步（默认关闭）；上传串行队列 + 5 次尝试预算 + 仅 Wi-Fi 门；`two_way` 模式支持恢复 NAS 独有照片（预览/勾选/导入）与本机删除后 best-effort 清理远端副本 |
| 字体与无障碍 | 内嵌 MiSans 子集（关于页署名，正文待 AppText 组件化后全量应用）；44vp 热区、中英文可读名称、跟随系统减少动画与深浅色 |
| 数据 | 单/多项目 ZIP 备份与恢复、恢复事务与中断收敛、应用私有存储统计 |
| 设置 | 中英文、浅色/深色/跟随系统、减少动画适配、可选 NAS 同步、诊断包、外部 GitHub 链接 |

## 权限与数据边界

应用声明前台精确/模糊定位权限与 `VIBRATE`（列表选择的触感反馈，系统自动授予），以及可选 NAS 同步所需的 `INTERNET` 与 `GET_NETWORK_INFO`。不声明 `CAMERA` 或广泛媒体读写权限。实际拍摄交给系统相机；保存到系统相册时使用 HarmonyOS 系统确认面板。GitHub 仓库链接交给外部浏览器。唯一的网络出口是用户主动配置并启用的 NAS 同步，默认关闭。
